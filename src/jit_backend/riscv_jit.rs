...
pub(crate) fn emit_ins<C: ContextObject>(
    ...
    jit.result.machine_instruction_count += 1;
}
...
pub(crate) fn emit_address_translation<C: ContextObject>(jit: &mut JitCompiler<C>, dst: Option<u8>, vm_addr: Value, len: u64, value: Option<Value>) {
    ...
    if jit.config.enable_address_translation {
        ...
        match len {
            1 => store(jit, OperandSize::S8, T5, REGISTER_SCRATCH, 0),
            2 => store(jit, OperandSize::S16, T5, REGISTER_SCRATCH, 0),
            4 => store(jit, OperandSize::S32, T5, REGISTER_SCRATCH, 0),
            8 => store(jit, OperandSize::S64, T5, REGISTER_SCRATCH, 0),
            _ => unreachable!(),
        }
    }
}
...
pub(crate) fn emit_subroutines<C: ContextObject>(
    ...
    load_immediate(jit, T5, jit.result.text_section.as_ptr() as i64); // REGISTER_SCRATCH = self.result.text_section;
    emit_ins(jit, RISCVInstruction::add(OperandSize::S64, REGISTER_MAP[0], T5, REGISTER_MAP[0])); // host_target_address += self.result.text_section;

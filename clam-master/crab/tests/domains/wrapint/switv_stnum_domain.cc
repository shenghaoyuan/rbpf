...
z_cfg_t *prog3(variable_factory_t &vfac) {
  ...
  /* APLAS'12 example
     ...
     The expected result at the end is x=[-8,-7]
  */
  
  // Defining program variables
  z_var x(vfac["x"], crab::INT_TYPE, 4);
  z_var y(vfac["y"], crab::INT_TYPE, 4);
  z_var nd(vfac["nd"], crab::INT_TYPE, 32);
  ...
  bb1_f >> ret;
  // adding statements
  entry.assign(y, z_number(-2));
  entry.havoc(nd);
  bb_nd_tt.assume(nd >= 1);
  bb_nd_tt.assign(x, z_number(0));
  bb_nd_ff.assume(nd <= 0);
  bb_nd_ff.assign(x, z_number(6));
  bb1_t.assume(x >= y);
  bb1_f.assume(x <= y - 1);
  bb2.sub(x, x, y);
  return cfg;
